...
def mainpage(request):
    ...
    top.location.href = '"""+str(CANVAS_HOME)+"""';
 </script>""")
...
def canvashome(request):
    ...
    response = render_response(request, 'bidding/mainpage.html',
        {'bids_auctions' : bids_auctions, 'tokens_auctions' : tokens_auctions,
         ...
         'pre_promoted_auctions':pre_promoted_auctions, 'facebook_user_id': member.facebook_id, 'promoted_auctions': promoted_auctions, 'tosintro':tosintro})
...
def promoted(request, auction_id):
    ...
    active = Auction.objects.filter(is_active=True).exclude(status__in=('waiting_payment', 'paid')).exclude(id__in = [p.id for p in PromotedAuction.objects.all()]).order_by('-status')
