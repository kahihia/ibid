...
def init(request):
    ...
    data = {
        u'auctions_token_available': auctions_token_available,
        u'auctions_token_finished': auctions_token_finished,
        u'auctions_token_my': auctions_token_my,
    
        u'auctions_bid_available': auctions_bid_available,
        u'auctions_bid_finished': auctions_bid_finished,
        u'auctions_bid_my': auctions_bid_my,
        u'display_name':member.display_name(),
        u'facebook_id':member.facebook_id,
        u'profile_foto_link':member.display_picture(),
        u'bids_left':member.bids_left,
        u'tokens_left':member.tokens_left
        }

    return HttpResponse(json.dumps(data))

def reloadTokens(request):
    """init info for the wiseDOM """
    member = request.user.get_profile()
...
def inviteRequest(request):
    ...
    invited = json.loads(request.POST.get('invited', '[]'))
...
def load_auction(request):
    """init info for the wiseDOM """
    
    #TODO try catch to avoid ugly error when admin is logged
    member = request.user.get_profile()
    
    auct = Auction.objects.get(id=int(request.GET.get('auction_id', int(request.POST.get('auction_id', 0)))))
    atype = request.GET.get('auction_type', int(request.POST.get('auction_type', 0)))
